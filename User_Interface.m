...
% Last Modified by GUIDE v2.5 11-Jul-2014 18:22:20
...
elseif (strcmp(FileName(end-2:end),'ust')==1)  
    ...
    if(mat_Fid ~= -1)
        ...
        set(handles.corrected_res,'String','');
...
%     end
    ...
    if (measurement_Count >= 1)
       ...
       set(handles.final_Results_Text,'String',{['Best Beta = ',num2str(best_beta)], ['Beta Confidence = ',num2str(beta_Confidence)],...
                       ['Final D_d = ',num2str(avg_Dia)],['Final Del_d = ',num2str(avg_del_D)], ['Final PAT = ',num2str(avg_PAT)],...
...
%                         long_dist_store =[long_dist_store(i:end) ];
            ...
            if  (a ~=0)
                negate_ecg = get(handles.neg_ECG,'value');
                if (negate_ecg==1)
                    ch2 = -1* ch2 ;
                end
                ...
                ECG_Data_Ch  = filtfilt(b_ecg_l, a_ecg_l, double( ecg_data(ecg_disp_start:end)));
                if (length (ecg_data)>(ecg_disp_start+4999))
                    ECG_Data_Ch = ( ECG_Data_Ch- min(ecg_data(ecg_disp_start+4999:end))) /(max(ecg_data(ecg_disp_start+4999:end))- min(ecg_data(ecg_disp_start+4999:end)));
                else
                    ECG_Data_Ch = (ECG_Data_Ch - min(ecg_data(ecg_disp_start:end))) /(max(ecg_data(ecg_disp_start:end))- min(ecg_data(ecg_disp_start:end)));
                end
...
%         
            ...
            [state1, wall1, wall2, on_Carotid, time_Arr, dist_Arr, dia_Arr, full_Cycle_Time_Array, full_Cycle_Dist_Array, full_Cycle_Lumen_Dia_Array, del_D, D_d,full_Cycle_ECG,full_Cycle_ECG_Time,PAT,AIx,AIx_Display_Loc] = track_Walls_wth_ECG(current_Frame, wall1, wall2, 50,TS);
%             long_dist_store(end) = dist_Arr(end);
            dist_time_store =[dist_time_store ;time_Arr(end)];
...
%                disp('one cycle extracted');
               if(on_Carotid==1)
                   ...
                   AIx_Store(measurement_Count) = AIx;
                   AIx_Display_Loc_Store(measurement_Count) = AIx_Display_Loc;
                   ...
                   [beta_Arr, ~, ~, best_beta, avg_Dia,avg_del_D,avg_PAT,SD_PAT, beta_Confidence, best_AIx] = calculate_Stiffness(del_D_Store(1:measurement_Count), D_d_Store(1:measurement_Count),PAT_Store(1:measurement_Count), SBP, DBP, AIx_Store(1:measurement_Count));
...
%                    cla(H2,(handles.mmt_Store));%[handles.AX h1 h2]=
                   ...
                   plot(handles.mmt_Store,full_Cycle_Time_Array(AIx_Display_Loc),full_Cycle_Dist_Array(AIx_Display_Loc),' Ok')
                   ...
                   set(handles.cycle_Info,'String',{['D_d =  ',num2str(D_d_Store(measurement_Count))],['Del_D = ', num2str(del_D_Store(measurement_Count))], ...
                       ['Beta = ', num2str(beta_Arr(measurement_Count))],['AIx =',num2str(AIx_Store(measurement_Count))],['PAT = ', num2str(PAT_Store(measurement_Count))],...
                   ...
                   set(handles.final_Results_Text,'String',{['Best Beta = ',num2str(best_beta)], ['Beta Confidence = ',num2str(beta_Confidence)],...
                       ['Best D_d = ',num2str(avg_Dia)],['Best Del_d = ',num2str(avg_del_D)], ['Best PAT = ',num2str(avg_PAT)],...
                       ['S.D. PAT = ',num2str(SD_PAT)],['Best AIx = ',num2str(best_AIx)]});
                   set (handles.beta_disp,'string',num2str(best_beta));
...
%                    [BX hb1 hb2]=plotyy(handles.JV_mmt_Store,full_Cycle_Time_Array, full_Cycle_Dist_Array,full_Cycle_ECG_Time,full_Cycle_ECG);
        ...
        if(mod(frame_Num,disp_no)==0)
            ...
            plot(handles.US_Frame_Plot,[wall1-1*points_Per_MM, wall1-1*points_Per_MM],[4,-4],'r');
            plot(handles.US_Frame_Plot,[wall1, wall1],[4,-4],'r');
            plot(handles.US_Frame_Plot,[wall2, wall2],[4,-4],'g');
            plot(handles.US_Frame_Plot,[wall2+1*points_Per_MM, wall2+1*points_Per_MM],[4,-4],'g');
...
%     profile viewer
    ...
    if (femoral== 0)
       ...
       c_PAT = PAT_Store;
       c_AIx = AIx_Store;
       c_AIx_Display_Loc = AIx_Display_Loc_Store;
       c_done=1; 
...
if (disp_femoral== 0)
    ...
    del_D_Store=c_del_D ;
    PAT_Store= c_PAT ;
    AIx_Store = c_AIx_Store;
	AIx_Display_Loc_Store = c_AIx_Loc_Store;
...
if(measurement_Count>0)
    ...
    SBP = str2double(get(handles.SBP_Edit,'String'));
    DBP = str2double(get(handles.DBP_Edit,'String'));
    [beta_Arr, ~, ~, best_beta, avg_Dia,avg_del_D,avg_PAT,SD_PAT, beta_Confidence, best_AIx] = calculate_Stiffness(del_D_Store(1:measurement_Count), D_d_Store(1:measurement_Count),PAT_Store(1:measurement_Count), SBP, DBP, AIx_Store);
    if(~isempty(AIx_Store))
    set(handles.final_Results_Text,'String',{['Best Beta = ',num2str(best_beta)], ['Beta Confidence = ',num2str(beta_Confidence)],...
                       ['Best D_d = ',num2str(avg_Dia)],['Best Del_d = ',num2str(avg_del_D)], ['Best PAT = ',num2str(avg_PAT)],...
                       ['S.D. PAT = ',num2str(SD_PAT)],['Best AIx = ',num2str(best_AIx)]});
    else
    ...
    if(~isempty(AIx_Store))
    set(handles.cycle_Info,'String',{['D_d =  ',num2str(D_d_Store(current_Display_Record_Index))],['Del_D = ', num2str(del_D_Store(current_Display_Record_Index))], ... 
                       ['Beta = ', num2str(beta_Arr(current_Display_Record_Index))],['AIx = ',num2str(AIx_Store(current_Display_Record_Index))],['PAT = ', num2str(PAT_Store(current_Display_Record_Index))],['cycle no =',num2str(current_Display_Record_Index),' /',num2str(measurement_Count)]...
                       ,['JV cycle no =',num2str(JV_current_Display_Record_Index),' /',num2str(JV_mmt_Count)]});
    else
    set(handles.cycle_Info,'String',{['D_d =  ',num2str(D_d_Store(current_Display_Record_Index))],['Del_D = ', num2str(del_D_Store(current_Display_Record_Index))], ... 
                       ['Beta = ', num2str(beta_Arr(current_Display_Record_Index))],['PAT = ', num2str(PAT_Store(current_Display_Record_Index))],['cycle no =',num2str(current_Display_Record_Index),' /',num2str(measurement_Count)]...
...
if(measurement_Count>0)
    ...
    hold on
    cla(handles.mmt_Store);
    time_Arr = time_Stamp_Store(1:data_Length_Store(current_Display_Record_Index),current_Display_Record_Index);
    plot(handles.mmt_Store,time_Arr, dist_Store((1:data_Length_Store(current_Display_Record_Index)),current_Display_Record_Index),'r');  
    try
    plot(handles.mmt_Store, time_Arr(AIx_Display_Loc_Store(current_Display_Record_Index)),dist_Store(AIx_Display_Loc_Store(current_Display_Record_Index),current_Display_Record_Index),' Ok');
    catch
        disp('AIx data not available');
    end
    temp_length= length(find(ECG_TS_Store(:,current_Display_Record_Index)~=0));
    plot(handles.mmt_Store,ECG_TS_Store(1:temp_length,current_Display_Record_Index),ECG_Store(1:temp_length,current_Display_Record_Index),'b');
    SBP = str2double(get(handles.SBP_Edit,'String'));
    DBP = str2double(get(handles.DBP_Edit,'String'));
    [beta_Arr, compliance_Arr, distensibility_Arr, best_beta, avg_Dia,avg_del_D,avg_PAT,SD_PAT, beta_Confidence,best_AIx] = calculate_Stiffness(del_D_Store(1:measurement_Count), D_d_Store(1:measurement_Count),PAT_Store(1:measurement_Count), SBP, DBP, AIx_Store);
    if(~isempty(AIx_Store))
    set(handles.cycle_Info,'String',{['D_d =  ',num2str(D_d_Store(current_Display_Record_Index))],['Del_D = ', num2str(del_D_Store(current_Display_Record_Index))], ... 
                       ['Beta = ', num2str(beta_Arr(current_Display_Record_Index))],['AIx = ',num2str(AIx_Store(current_Display_Record_Index))],['PAT = ', num2str(PAT_Store(current_Display_Record_Index))],['cycle no =',num2str(current_Display_Record_Index),' /',num2str(measurement_Count)]...
                       ,['JV cycle no =',num2str(JV_current_Display_Record_Index),' /',num2str(JV_mmt_Count)]});
...
if(c_measurement_Count >= 1)
    [beta_Arr, compliance_Arr, distensibility_Arr, best_beta, avg_Dia,avg_del_D,avg_PAT,SD_PAT, beta_Confidence, best_AIx] = calculate_Stiffness(c_del_D(1:c_measurement_Count), c_D_d(1:c_measurement_Count),c_PAT(1:c_measurement_Count), SBP, DBP, c_AIx);
    ...
    [beta_Arr, compliance_Arr, distensibility_Arr, best_beta, avg_Dia,avg_del_D,c_avg_PAT,c_SD_PAT, beta_Confidence, best_AIx] = calculate_Stiffness(c_del_D(1:c_measurement_Count), c_D_d(1:c_measurement_Count),PAT_carotid(1:count), SBP, DBP, c_AIx);
else
...
%     best_beta
    ...
    C_SBP = DBP + delta_p;
    [beta_Arr, compliance_Arr, distensibility_Arr, best_beta_calcu, avg_Dia,avg_del_D,c_avg_PAT,c_SD_PAT, beta_Confidence, best_AIx] = calculate_Stiffness(c_del_D(1:c_measurement_Count), c_D_d(1:c_measurement_Count),PAT_carotid(1:count), C_SBP , DBP, c_AIx);
    set(handles.pwv_result,'String',{['carotid PAT=',num2str(c_avg_PAT),' ms | ',num2str(c_SD_PAT)],['femoral PAT=',num2str(f_avg_PAT),' ms | ',num2str(f_SD_PAT)],['time delay=  ',num2str(time_diff),' ms']...
        , ['car HR=',num2str(c_HR),'   fem HR=',num2str(f_HR)] }); % ['del_P calcu = ',num2str(delta_p),' mm/Hg'], ['Car Beta calcu = ',num2str(best_beta_calcu)],['Carotid Beta = ',num2str(best_beta)], ,['PWV =  ',num2str(pwv),' m/s']
    if(~isempty(c_AIx))
        set(handles.corrected_res,'String',{['CBP = ',num2str(C_SBP),'/ ',num2str(DBP)], ['C_Beta = ',num2str(best_beta),'/ ',num2str(best_beta_calcu)], ['Final AIx = ',num2str(best_AIx)]});
    else
        set(handles.corrected_res,'String',{['CBP = ',num2str(C_SBP),'/ ',num2str(DBP)], ['C_Beta = ',num2str(best_beta),'/ ',num2str(best_beta_calcu)], ['Final AIx = ','Not Available']});
    end
    if (abs(c_HR - f_HR)>=10)
    ...
    set(handles.pwv_result,'String',{['carotid PAT=',num2str(c_avg_PAT),' ms | ',num2str(c_SD_PAT)],['femoral PAT=',num2str(f_avg_PAT),' ms | ',num2str(f_SD_PAT)],['car HR=',num2str(c_HR),'fem HR=',num2str(f_HR)],[ 'measurement not yet completed']}); %['Carotid Beta = ',num2str(best_beta)],
...
result_table= {'name:',patient_name;'age',age;'hight',hight; 'weight',weight; 'gender',gender;'posture',posture;...
                ...
                'Car Beta calcu',best_beta_calcu; 'carotid HR=',c_HR;'fememoral HR=',f_HR; 'best AIx', best_AIx}%  'SD carotid PAT(ms)',c_SD_PAT; 'SD femoral PAT(ms)',f_SD_PAT }
...
function [state, wall1, wall2, on_Carotid, time_Array, distension_Array, dia_Array, full_Cycle_Time_Array, full_Cycle_Dist_Array, full_Cycle_Lumen_Dia_Array, del_D, D_d,full_Cycle_ECG,full_Cycle_ECG_Time,PAT,AIx,AIx_Display_Loc] = track_Walls_wth_ECG(frame, Wall1, Wall2, pls_Rate,TS)
global last_Track_Frame;
global PW_Wave;
global DW_Wave;
global TS_Wave;
...
if(current_State ==2)
    ...
    PW_Wave = [Wall1];
    DW_Wave = [Wall2];
    TS_Wave = [prev_Time_Stamp*ecg_on];  %Time stamps are initiated at 0 if ecg is off
    current_State=3;
...
if(ecg_on~=0)
    ...
    TS_Wave = [TS_Wave;TS_Wave(end)+1000/pls_Rate];
end
...
if(valid_Position == 0)
    ...
    if(check_Walls_Present(frame,PW_Wave(end),DW_Wave(end))== 0)
        ...
        wall1= PW_Wave(end);
        wall2= DW_Wave(end);
         [dia_Wave(end,1),dia_Confidence(end,1),PW_Inner_Loc, DW_Inner_Loc] = find_Dia_L_Fit(frame,PW_Wave(end,1),DW_Wave(end,1),samples_Per_Frame,points_Per_MM);
        if (ecg_on==1)
            [state_Change, on_Carotid, change_In_Dia, diastolic_Diameter, distension_Wave,sub_TS_Wave,ecg_sub_Wave,sub_tstamp_ecg,PAT,AIx,AIx_Display_Loc] = check_Cycles_ECG_Method();
        else
            [state_Change, on_Carotid, change_In_Dia, diastolic_Diameter, distension_Wave,sub_TS_Wave,AIx,AIx_Display_Loc] = check_Cycles_ZC_Method();
        end
        ...
        if(current_State==4)
            ...
            AIx_Display_Loc = AIx_Display_Loc - sub_TS_Wave(1)+1;
            del_D =  change_In_Dia;
            D_d = diastolic_Diameter;
            full_Cycle_ECG = ecg_sub_Wave;
            full_Cycle_ECG_Time = ecg_on*(sub_tstamp_ecg -sub_TS_Wave(1));
            PW_Wave = PW_Wave - (PW_Wave (end)- PW_Inner_Loc);
            DW_Wave = DW_Wave - (DW_Wave (end)- DW_Inner_Loc);
        end
    end
end
...
valid_Positions = 1;
ROI1 = (last_PW - round(1*pts_Per_MM) : last_PW+ round(0*pts_Per_MM));   % Modified to track only 0.5 mm
...
ROI2 = (last_DW - round(0*pts_Per_MM) : last_DW + round(1*pts_Per_MM));
...
if(~isempty(V))
    ...
    if(I>0)
        ...
        Wall1=V(I,1);
        Wall2=V(I,2);
        [~,~,Wall1, Wall2] = find_Dia_L_Fit(frame,Wall1,Wall2,samples_Per_Frame,points_Per_MM);
    end
end
...
if((Wall2-Wall1)/points_Per_MM < min_Wall_Dia || (Wall2-Wall1)/points_Per_MM > max_Wall_Dia || Wall1/points_Per_MM< 3 || (length(frame)-Wall2)/points_Per_MM < 3)
    ...
    Slot1= frame(Wall1-round(2*points_Per_MM/2):Wall1+round(0*points_Per_MM/2));
    Slot2= frame(Wall2-round(0*points_Per_MM/2):Wall2+round(2*points_Per_MM/2));
...
function [ diameter_Lumen,confidence, Inner_Left_Wall_Loc, Inner_Right_Wall_Loc] = find_Dia_L_Fit(frame,Wall1,Wall2,No_Of_Samples,points_Per_mm)
%UNTITLED Summary of this function goes here
%   Detailed explanation goes here
global last_Dia;
global b_L_Fit;
...
Inner_Left_Wall_Loc = mid_Pt-left_Intima_Pt;
Inner_Right_Wall_Loc = mid_Pt+right_Intima_Pt;

function [ state_Change,  on_Carotid, Del_D, D_d, dist_Wave,sub_TS_Wave,AIx,AIx_Display_Loc] = check_Cycles_ZC_Method()
%UNTITLED2 Summary of this function goes here
%   Detailed explanation goes here
global PW_Wave ;
global DW_Wave;
...
%        sub_TS_Wave_Interp = [sub_TS_Wave(1):1:sub_TS_Wave(end)];                     %%%Ashish edit for AIx
%        dist_Wave = interp1(sub_TS_Wave,dist_Wave,sub_TS_Wave_Interp, 'spline');          %%%Ashish edit for AIx
%        sub_TS_Wave = sub_TS_Wave_Interp;                                                 %%%Ashish edit for AIx
%        [AIx,AIx_Display_Loc] = calculate_AIx(sub_TS_Wave', dist_Wave');                    %%%Ashish edit for AIx
        [AIx,AIx_Display_Loc] = calculate_AIx_No_Interpolation(sub_TS_Wave, dist_Wave);
       state_Change=1;
...
elseif((length(PW_Wave) > 1.5*pulse_Rate)&& (TS_on==0))
    ...
    TS_Wave = TS_Wave (end-floor(1.5*pulse_Rate)+1:end);
    dia_Confidence = dia_Confidence(end-floor(1.5*pulse_Rate)+1:end);
end

function [ state_Change,  on_Carotid, Del_D, D_d, dist_Wave,sub_TS_Wave,ecg_sub_Wave,sub_tstamp_ecg,PAT, AIx,AIx_Display_Loc] = check_Cycles_ECG_Method()
%UNTITLED2 Summary of this function goes here
%   Detailed explanation goes here
global PW_Wave ;
global DW_Wave;
...
%            on_Carotid =-1;
       ...
       if( (C_Dist_Dia(1,1) >0 ||  femoral == 1)&& on_Carotid ==1)
           ...
           dia_Sub_Wave = corrected_Dia(dia_Sub_Wave,confidence_Sub_Wave, sub_Wave);
           D_d = dia_Sub_Wave(distension_Min_Point,1);
           sub_TS_Wave_inter= [sub_TS_Wave(1):1:sub_TS_Wave(end)]';                     %%%Ashish edit for AIx
%            dist_Wave = (interp1(sub_TS_Wave,dist_Wave,sub_TS_Wave_inter, 'spline'));          %%%Ashish edit for AIx
%            [AIx,AIx_Display_Loc] = calculate_AIx(sub_TS_Wave_inter, dist_Wave);     %%%Ashish edit for AIx
%            sub_TS_Wave = sub_TS_Wave_inter;
           
           [AIx,AIx_Display_Loc] = calculate_AIx_No_Interpolation(sub_TS_Wave, dist_Wave);     %%%Ashish edit for AIx
           
           [~,max_Loc_int] = max(dist_Wave);
           [~,min1_Loc_int] = min(dist_Wave(1:max_Loc_int,1));
           diff_dist = diff(dist_Wave,2);
...
function [AIx,shoulder_Point] =calculate_AIx(dist_Interpolated_X, dist_Interpolated_Y)   %%%Ashish edit for AIx
    AIx = 100;
    dist_Interpolated_Y = smooth(dist_Interpolated_Y,round(length(dist_Interpolated_Y)/10));
    shoulder_Point = dist_Interpolated_X(1);
    first_Derivative = diff(dist_Interpolated_Y);
    [~,systolic_Onset] = max(first_Derivative);
    mx = max(dist_Interpolated_Y);
    mn = min(dist_Interpolated_Y);
    third_Derivative = [0;diff(dist_Interpolated_Y,3);0;0];
    sign_Derivative_of_third_Derivative= sign([0;diff(sign(third_Derivative))]);
    I = find(sign_Derivative_of_third_Derivative<0); %<0 for pos to neg
    if(~isempty(I))
        Loc = I(find(I>systolic_Onset & dist_Interpolated_Y(I)> mx-(mx-mn)/2,1,'first'));
        if(~isempty(Loc))
            [~,largest_Value_Index] = max(dist_Interpolated_Y);
            temp = abs(Loc - largest_Value_Index);
            [~,I] = min(temp);
            Aug_I_Sign = sign(largest_Value_Index-Loc(I));
            AIx = Aug_I_Sign*(max(dist_Interpolated_Y)-dist_Interpolated_Y(Loc(I)))./(max(dist_Interpolated_Y)-min(dist_Interpolated_Y));
            shoulder_Point = dist_Interpolated_X(Loc(I));
        else
            AIx = 100;
            shoulder_Point = dist_Interpolated_X(1);
        end
...
function [AIx,shoulder_Point] =calculate_AIx_No_Interpolation(dist_X, dist_Y)   %%%Ashish edit for AIx
    AIx = 100;
    shoulder_Point = dist_X(1);
    first_Derivative_X = conv(dist_X,[0.5 0.5]);
    first_Derivative_X = first_Derivative_X(2:end-1);
    first_Derivative_Y = diff(dist_Y,1)./first_Derivative_X;
    first_Derivative_Root_Locations = interp1(first_Derivative_X,first_Derivative_Y, dist_X,'spline');
    [~,systolic_Onset] = max(first_Derivative_Root_Locations);
    mx = max(dist_Y);
    mn = min(dist_Y);
    second_Derivative_X = conv(first_Derivative_X,[0.5 0.5]);
    second_Derivative_X = second_Derivative_X(2:end-1);
    third_Derivative_X = conv(second_Derivative_X,[0.5 0.5]);
    third_Derivative_X = third_Derivative_X(2:end-1);
    third_Derivative_Y = diff(dist_Y,3)./third_Derivative_X;
    third_Derivative_Root_Locations = interp1(third_Derivative_X,third_Derivative_Y, dist_X);
    sign_Derivative_of_third_Derivative= sign([diff(sign(third_Derivative_Root_Locations)); 0]);
    I = find(sign_Derivative_of_third_Derivative<0); %<0 for pos to neg
    if(~isempty(I))
        Loc = I(find(I>systolic_Onset & dist_Y(I)> mx-(mx-mn)/2,1,'first'));
        if(~isempty(Loc))
            [~,largest_Value_Index] = max(dist_Y);
            temp = abs(Loc - largest_Value_Index);
            [~,I] = min(temp);
            Aug_I_Sign = sign(largest_Value_Index-Loc(I));
            AIx = Aug_I_Sign*(max(dist_Y)-dist_Y(Loc(I)))./(max(dist_Y)-min(dist_Y));
            shoulder_Point = Loc(I)+dist_X(1);
        else
            AIx = 100;
            shoulder_Point = 1+dist_X(1);
        end
    end
...
function [beta_Arr, compliance_Arr, distensibility_Arr, best_beta, avg_Dia,avg_del_D,avg_PAT,SD_PAT, beta_Confidence, best_AIx] = calculate_Stiffness(del_D_Arr, D_d_Arr,PAT_Arr, SBP, DBP, AIx_Arr)
global stop_All;
global running;
global live;
global req_no_cycle ;
...
best_AIx = -1;
try
    best_AIx = median(AIx_Arr(find(AIx_Arr~=100)));
catch
    disp('AIx_Data_Not_Avaibale');
end
...
if ((length(sub_ecg_data) >=200) && max (abs(sub_ecg_data)) ~=0)
    ...
    [Rpeak]=ecg_Rpeakdet(sub_ecg_data, 0.7);
    pospeak = Rpeak(:, 1);
...
function corrected_res_Callback(hObject, eventdata, handles)
% hObject    handle to corrected_res (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of corrected_res as text
%        str2double(get(hObject,'String')) returns contents of corrected_res as a double


% --- Executes during object creation, after setting all properties.
function corrected_res_CreateFcn(hObject, eventdata, handles)
% hObject    handle to corrected_res (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in neg_ECG.
function neg_ECG_Callback(hObject, eventdata, handles)
% hObject    handle to neg_ECG (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of neg_ECG
